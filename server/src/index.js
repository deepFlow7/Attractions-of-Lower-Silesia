...
app.get('/profile', async (req, res) => {
  ...
  if (req.session.user) {
    res.json({
      authenticated: true, blocked: req.session.blocked, user: req.session.user,
...
app.get('/completed_challenges', async (req, res) => {
  try {
    var challenges = await db.get_completed_challenges(req.session.user.id);
    res.json(challenges);
  }
  catch (error) {
...
app.post('/start_challenge/:challenge_id', async (req, res) => {
  try {
    await db.start_challenge(req.params['challenge_id'], req.session.user.id);
    res.json({ success: true });
  } catch (error) {
    res.json({ success: false, error: error });
  }
...
app.get('/takes_part_in_challenge/:challenge_id', async (req, res) => {
  try {
    const rows = await db.takes_part_in_challenge(req.session.user.id, req.params['challenge_id']);
...
app.post('/visit_challenge_attraction/:challenge_id/:attraction_id', async (req, res) => {
  try {
    await db.visit_challenge_attraction(req.session.user.id, req.params['challenge_id'],
...
app.get('/challenge/visited_attractions/:challenge_id', async (req, res) => {
  try {
    const rows = await db.get_visited_challenge_attractions(req.session.user.id, req.params['challenge_id']);
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error });
  }
...
app.get('/attraction/is_favourite/:attr_id', async (req, res) => {
  try {
    const favourite = await db.isFavourite(req.params["attr_id"], req.session.user.id);
...
app.get('/attraction/is_to_visit/:attr_id', async (req, res) => {
  try {
    const to_visit = await db.isToVisit(req.params["attr_id"], req.session.user.id);
...
app.get('/attraction/rating/:attr_id/user', async (req, res) => {
  try {
    const rating = await db.get_user_rating(req.session.user.id, req.params["attr_id"]);
    res.json({ rating: rating });
  }
  catch (error) {
